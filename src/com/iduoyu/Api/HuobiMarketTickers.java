...
public class HuobiMarketTickers {
    ...
    public static void huobiMarketTickers(){
...
//            trustEveryone();
            ...
            for (int i = 0; i < data.size(); i++) {
                ...
                if (m.find()){  //查找如果存在就是true，不存在就是false，下面可以进行数据操作
                    ...
                    double vol = data.getJSONObject(i).getDouble("vol");//24小时成交额-------入库
                    vol=(double)Math.round(vol*100)/100;
                    double amount = data.getJSONObject(i).getDouble("amount");//24小时成交量-------入库
                    ...
                    if (DetailData.getDetailData(detailSql,coin)==null){
                        ...
                        UpdateData.UpdateOneData(updataSql,coinpriceclose,amount,vol,change,coin);
                    }
